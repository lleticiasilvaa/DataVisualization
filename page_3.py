...
with st.expander(f"{len(st.session_state.apis)} APIs | Price per 1M tokens", expanded=True):
    if len(st.session_state.apis) > 0:
        ...
        st.info("No API registered yet.")

st.divider()

st.markdown("### Register API")
...
with st.form("form_cadastro"):
    ...
    if cadastrar and nome:
        ...
        st.success(f"API '{nome}' successfully registered!")
        st.rerun()
